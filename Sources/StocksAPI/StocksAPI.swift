...
public struct StocksAPI {
    ...
    public func fetchChartData(symbol:String,range:ChartRange) async throws -> ChartData?{
        ...
        urlComponents.queryItems = [.init(name: "range", value: range.rawValue),
    ...
    public  func searchTickets(query:String, isEquityOnly:Bool = true) async throws -> [Ticker]{
    ...
    public func fetchQuotes(symbols:String) async throws -> [Quote]{
        ...
        let (response,statucCode):(QuoteResponse,Int) = try await fetch(url: url)
        if let error = response.error{
            throw APIServiceError.httpStatusCodeFailed(statusCode: statucCode, error: error)
        }
